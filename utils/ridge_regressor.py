...
class RidgeRegressor:
    ...
    def __init__(self, train, test, cv_ratio=0.2):
        self.train = train
        self.test = test
    ...
    def predict(self):
        ...
        train_x = self.train[:, :-1]
        train_y = self.train[:, -1]
        test_x = self.test[:, :-1]
        test_y = self.test[:, -1]

        train_x, cv_x, train_y, cv_y = train_test_split(train_x, train_y, test_size=self.cv_ratio)

        verbose = True
        if verbose:
            print('>>> Shape of train matrix: {0} x {1}'.format(*train_x.shape))
            print('>>> Shape of cv matrix: {0} x {1}'.format(*cv_x.shape))
            print('>>> Shape of test matrix: {0} x {1}\n'.format(*test_x.shape))
        ...
        best_predictor.fit(train_x, train_y)
    ...
    def predict_from_sparse(self, vectorize_train_func, vectorize_test_func):
        ...
        print('>>> Ridge sparse model: R2 of test dataset: {0}'.format(r2_score(test_y, test_yhat)))
