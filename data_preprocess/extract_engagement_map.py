...
""" Scripts to extract engagement map.
# Fields:
# 'id', 'publish', 'duration', 'definition', 'category', 'detect_lang', 'channel', 'topics',
# 'view@30', 'watch@30', 'wp@30', 'days', 'daily_view', 'daily_watch'

Input: input_doc embedded in main function
Usage: python extract_engagement_map.py
Example: python extract_engagement_map.py
Time: ~8M
"""
...
def get_engagement_stats_from_file(filepath):
    with open(filepath, 'r') as fin:
        ...
        for line in fin:
            ...
            _, _, _, _, _, _, _, wp30, _ = dump.split('\t', 8)
...
if __name__ == '__main__':
    ...
    if to_check:
        print('-' * 79)
        for i in xrange(len(duration_splits)):
            print('bin with duration <= {0}; number of videos in bin: {1}'.format(duration_splits[i],
                                                                                  len(wp_bin_matrix[i])))
        print('bin with duration <= inf; number of videos in bin: {0}'.format(len(wp_bin_matrix[-1])))
        print('num of bins: {0}'.format(len(wp_bin_matrix)))
        print('-' * 79)
