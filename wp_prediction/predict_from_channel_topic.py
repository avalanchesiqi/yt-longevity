        ...
        for f in files:
            ...
            if f in train_channel_cnt_map and train_channel_cnt_map[f] > 4:
                ...
                with open(train_data_path, 'r') as fin:
                    for line in fin:
                        row = np.zeros(topic_cnt + 3)
                        _, duration, dump = line.rstrip().split('\t', 2)
                        _, topics, _, true_wp, wp_percentile = dump.rsplit('\t', 4)
                        topics = topics.split(',')
                        for topic in topics:
                ...
                test_dut_wp = ridge_regression(train_log, train_percentile, test_log, 'percentile')
                log_percentile_value.extend(test_dut_wp)
            ...
            # if not, set as 'NA'
                ...
                test_dut_wp = ['NA'] * test_num

            for vid, pred_wp in zip(test_vids, test_dut_wp):
