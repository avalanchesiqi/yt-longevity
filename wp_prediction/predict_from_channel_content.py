        ...
        for f in files:
            ...
            if f in train_channel_cnt_map and train_channel_cnt_map[f] > 4:
                ...
                with open(train_data_path, 'r') as fin:
                    for line in fin:
                        row = np.zeros(2+category_cnt+lang_cnt+3)
                        _, duration, definition, category, lang, _, _, _, true_wp, wp_percentile = line.rstrip().split('\t')
                        if definition == '0':
                            row[0] = 1
                        else:
                            row[1] = 1
                ...
                with open(os.path.join(subdir, f), 'r') as fin:
                    for line in fin:
                        row = np.zeros(2+category_cnt+lang_cnt+2)
                        vid, duration, definition, category, lang, _, _, _, true_wp, _ = line.rstrip().split('\t')
                        if definition == '0':
                            row[0] = 1
                        else:
                            row[1] = 1
                ...
                test_duc_wp = ridge_regression(train_log, train_percentile, test_log, 'percentile')
                log_percentile_value.extend(test_duc_wp)
            ...
            # if not, set as 'NA'
                ...
                test_duc_wp = ['NA'] * test_num

            for vid, pred_wp in zip(test_vids, test_duc_wp):
