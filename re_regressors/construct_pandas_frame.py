...
Example rows
           Vid   True  Duration   Content     Topic    CTopic       CPS        All       CSP  Test duration  
...
if __name__ == '__main__':
    ...
    if not os.path.exists(dataframe_path):
        ...
        test_duration_data_f = pd.DataFrame(test_duration.items(), columns=['Vid', 'Test duration'])
        data_f = true_data_f.merge(duration_data_f, on='Vid').merge(content_data_f, on='Vid')\
            ...
            .merge(all_data_f, on='Vid').merge(per_channel_data_f, on='Vid').merge(test_duration_data_f, on='Vid')
